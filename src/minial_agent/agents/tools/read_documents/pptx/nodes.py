...
from minial_agent.agents.tools.read_documents.pptx.prompts import PAGE_SCAN_PROMPT
...
def scan_pptx_pages_node(
    ...
    *,
    ...
    evidence, scanned_pages, is_sufficient = scan_pptx_pages(
        state["artifact"],
    ...
    emit_read_step(
        file_type="pptx",
        step="scan",
        message=f"PPTX 페이지 {scanned_pages}개 스캔을 완료했습니다.",
        status="completed",
        details={
            "path": state["artifact"].visible_name,
...
def build_pptx_answer_node(state: PptxReadState) -> PptxReadState:
    emit_read_step(
        file_type="pptx",
        step="answer",
