...
from minial_agent.agents.domain.office_file_editor.subagents.editor_xlsx.workflow.edit.nodes import apply_xlsx_edit_spec, build_xlsx_edit_spec, register_xlsx_edit_result, resolve_xlsx_artifact
from minial_agent.agents.domain.office_file_editor.subagents.editor_xlsx.workflow.edit.state import XlsxEditState


def build_xlsx_edit_workflow(
    workspace: UploadWorkspace,
    *,
    operation_selector: OperationSelector | None = None,
    slot_filler: SlotFiller | None = None,
):
    graph = StateGraph(XlsxEditState)
    graph.add_node(
    ...
    graph.add_node(
        ...
        lambda state: _build_xlsx_edit_spec(
            state,
    ...
    graph.add_node(
        ...
        lambda state: _apply_xlsx_edit_spec(state, workspace=workspace),
    )
    graph.add_node(
        "register_xlsx_edit_result",
        lambda state: _register_xlsx_edit_result(state, workspace=workspace),
    )
    graph.add_edge(START, "resolve_xlsx_artifact")
    graph.add_edge("resolve_xlsx_artifact", "build_xlsx_edit_spec")
    graph.add_edge("build_xlsx_edit_spec", "apply_xlsx_edit_spec")
    graph.add_edge("apply_xlsx_edit_spec", "register_xlsx_edit_result")
    graph.add_edge("register_xlsx_edit_result", END)
...
def _build_xlsx_edit_spec(
    ...
    *,
    ...
    emit_edit_step(
        file_type="xlsx",
        step="spec",
        message="XLSX 수정 방법을 정리합니다.",
        summary={"path": state["artifact"].visible_name},
    )
    edit_spec = build_xlsx_edit_spec(
    ...
    emit_edit_step(
        file_type="xlsx",
        step="spec",
        message=f"XLSX 수정 작업을 {edit_spec['operation']} 방식으로 준비했습니다.",
        status="completed",
...
def _apply_xlsx_edit_spec(
    ...
    *,
    ...
    changed_items = apply_xlsx_edit_spec(
        artifact=state["artifact"],
    ...
    emit_edit_step(
        file_type="xlsx",
        step="apply",
        message=f"XLSX 수정 적용을 완료했습니다. 변경 항목 {len(changed_items)}개를 만들었습니다.",
        status="completed",
...
def _register_xlsx_edit_result(
    state: XlsxEditState,
    *,
    workspace: UploadWorkspace,
) -> XlsxEditState:
    emit_edit_step(
        file_type="xlsx",
        step="register",
        message="XLSX 수정본을 등록하고 다운로드 정보를 준비합니다.",
        summary={"path": state["artifact"].visible_name},
    )
    result = register_xlsx_edit_result(
        workspace=workspace,
        artifact=state["artifact"],
...
def edit_xlsx(file_path: str, instruction: str, runtime: ToolRuntime) -> str:
    """Edit an XLSX workbook."""
    try:
        workflow = build_xlsx_edit_workflow(workspace_from_tool_runtime(runtime))
