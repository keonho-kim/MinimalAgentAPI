...
from minial_agent.agents.domain.office_file_agent.subagents.agent_docx.workflow.edit.nodes import apply_docx_edit_spec, build_docx_edit_spec, register_docx_edit_result, resolve_docx_artifact
from minial_agent.agents.domain.office_file_agent.subagents.agent_docx.workflow.edit.state import DocxEditState
...
def build_docx_edit_workflow(
    ...
    *,
    ...
    graph.add_node(
        ...
        lambda state: _build_docx_edit_spec(
            state,
    ...
    graph.add_node(
        ...
        lambda state: _apply_docx_edit_spec(state, workspace=workspace),
    )
    graph.add_node(
        "register_docx_edit_result",
        lambda state: _register_docx_edit_result(state, workspace=workspace),
    )
...
def _build_docx_edit_spec(
    ...
    *,
    ...
    emit_edit_step(
        file_type="docx",
        step="spec",
        message="DOCX 수정 방법을 정리합니다.",
        summary={"path": state["artifact"].visible_name},
    )
    edit_spec = build_docx_edit_spec(
    ...
    emit_edit_step(
        file_type="docx",
        step="spec",
        message=f"DOCX 수정 작업을 {edit_spec['operation']} 방식으로 준비했습니다.",
        status="completed",
...
def _apply_docx_edit_spec(
    ...
    *,
    ...
    changed_items = apply_docx_edit_spec(
    ...
    emit_edit_step(
        file_type="docx",
        step="apply",
        message=f"DOCX 수정 적용을 완료했습니다. 변경 항목 {len(changed_items)}개를 만들었습니다.",
        status="completed",
...
def _register_docx_edit_result(
    state: DocxEditState,
    *,
    workspace: UploadWorkspace,
) -> DocxEditState:
    emit_edit_step(
        file_type="docx",
        step="register",
        message="DOCX 수정본을 등록하고 다운로드 정보를 준비합니다.",
        summary={"path": state["artifact"].visible_name},
    )
    result = register_docx_edit_result(
        workspace=workspace,
        artifact=state["artifact"],
...
@tool
def edit_docx(file_path: str, instruction: str, runtime: ToolRuntime) -> str:
    """Edit a DOCX file."""
    try:
        workflow = build_docx_edit_workflow(workspace_from_tool_runtime(runtime))
